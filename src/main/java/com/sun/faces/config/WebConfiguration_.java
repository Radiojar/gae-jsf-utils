...
/** Class Documentation */
public class WebConfiguration_ {
    ...
    // Key under which we store our WebConfiguration_ instance.
    ...
    private WebConfiguration_(ServletContext servletContext) {
    ...
    /**
     * Return the WebConfiguration_ instance for this application passing
     * the result of FacesContext.getCurrentInstance().getExternalContext()
     * to {@link #getInstance(javax.faces.context.ExternalContext)}.
     * @return the WebConfiguration_ for this application or <code>null</code>
     *  if no FacesContext is available.
     */
    public static WebConfiguration_ getInstance() {
    ...
    /**
     * Return the WebConfiguration_ instance for this application.
     * @param extContext the ExternalContext for this request
     * @return the WebConfiguration_ for this application
     */
    public static WebConfiguration_ getInstance(ExternalContext extContext) {

        WebConfiguration_ config = (WebConfiguration_) extContext.getApplicationMap()
              .get(WEB_CONFIG_KEY);
        if (config == null) {
     ...
     * @return the WebConfiguration_ for this application or <code>null</code>
     *  if no WebConfiguration_ could be located
     */
    public static WebConfiguration_ getInstance(ServletContext servletContext) {

        WebConfiguration_ webConfig = (WebConfiguration_)
              servletContext.getAttribute(WEB_CONFIG_KEY);

        if (webConfig == null) {
            webConfig = new WebConfiguration_(servletContext);
     ...
     * this WebConfiguration_ instance
     */
    ...
    private interface DeprecationLoggingStrategy {
        ...
        boolean shouldBeLogged(WebConfiguration_ configuration);
    ...
    private static class FaceletsConfigParamLoggingStrategy implements DeprecationLoggingStrategy {
        ...
        public boolean shouldBeLogged(WebConfiguration_ configuration) {
    ...
    private class DeferredParameterLoggingAction implements DeferredLoggingAction {
        ...
        public void log() {
            ...
            if (WebConfiguration_.LOGGER.isLoggable(loggingLevel)) {
                DeprecationLoggingStrategy strategy = parameter.getDeprecationLoggingStrategy();
                if (strategy != null && strategy.shouldBeLogged(WebConfiguration_.this)) {
                    WebConfiguration_.LOGGER.log(loggingLevel, logKey, params);
                }
            }
